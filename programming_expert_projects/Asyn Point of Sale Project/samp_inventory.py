...
class Inventory:
    ...
    def _item_dict(self):
        ...
        for category in self.catalogue:
            if isinstance(self.catalogue[category], list):
                for item in self.catalogue[category]:
                    new_item = item.copy()
                    new_item["category"] = category
                    new_item["subcategory"] = None
                    self.items[item["id"]] = new_item
            else:
                for subcategory in self.catalogue[category]:
                    for item in self.catalogue[category][subcategory]:
                        new_item = item.copy()
                        new_item["category"] = category
                        new_item["subcategory"] = subcategory
                        new_item["name"] = f"{new_item['size']} {subcategory}"
                        self.items[item["id"]] = new_item
    ...
    def _verify_id(func):
        async def wrapper(self, item_id):
            if item_id not in self.stock:
                raise ValueError(
                    f"The item with ID {item_id} is not in the catalogue. Input value less than {len(self.stock) + 1}"
                )
    ...
    @_verify_id
    async def stock_return_value(self, item_id):
        if await self.get_stock(item_id) <= 0:
            return False
        return True
